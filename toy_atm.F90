...
CONTAINS
  ...
  SUBROUTINE main_atm(comm)
    ...
    write(*, *) "atm: Size cell_to_vertix: ", size(cell_to_vertex, dim=1)
...
!        x_cells, y_cells, cell_point_id )
  ...
  subroutine read_grid_from_netcdf(filename, num_vertices_lon, num_vertices_lat, num_cells, &
    ...
    write(*, *) 'ATM: ndim, nvar, natt, k_un:', ndim, nvar, natt, k_un
    ...
    write(*, *) "ATM: new coords lon = ", num_vertices_lon, " new coords lat = ", num_vertices_lat
    ...
    do k = 1, nvar
       ...
       write(*,'(a,I3,a,a,a,I3,a,I3,a,I3)') "ATM: variable id: ", k, " varname: ", trim(name), ", vardatatype: ", &
            xtype, ", vardim: ", ndims, ", variable number of attributes: ", natts

       if(index(trim(name), 'lon') /= 0) then
          write(*, *) "ATM: lon found"
          allocate(x_vertices(num_vertices_lon))
          write(*, *) "ATM: shape x_vertices: ", shape(x_vertices)
       ...
       if(index(trim(name), 'lat') /= 0) then
          ...
          write(*, *) "ATM: shape y_vertices: ", shape(y_vertices)
...
! Allocate and fill the vertex arrays (for longitude and lattitude
    ...
    write(*, *) "ATM: attr_grid = ", attr_grid
    ! Output of the attributes
    write(*, *) "----- ATM: Attributes -----"
    if(natt > 0) then
  ...
  !=============================================================================
  subroutine parse_attr_list( ncid, n_attr_loc, varid, attr_grid, attr_grid_total )
    use netcdf

    character(len = 256) :: c1d
    character(len = 128) :: name
    character(len = 1000), intent(inout) :: attr_grid
    character(len = 2000), intent(out) :: attr_grid_total

    integer(kind = 4), intent(in) :: ncid, n_attr_loc, varid
    ...
    do k = 1, n_attr_loc
       ...
       if (xtype == NF90_CHAR) then
          ...
          write(*, '("ATM name attr: ",a," c1d: ", a)') trim(name), trim(attr_grid)
          ...
          write(*, *) "ATM: xtype attr NF90_INT:", xtype
          ...
          write(*, *) "ATM: xtype attr NF90_FLOAT:", xtype
          ...
          write(*, *) "ATM: xtype attr NF90_DOUBLE:", xtype
       endif
    enddo
